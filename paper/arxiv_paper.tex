...
The experiments reported here are backed by executable benchmark scripts, task
JSON files, figure-generation code, and the result JSON artifacts used to
populate the tables and figures. The public repository is:
\url{https://github.com/ranausmanai/LLMEmotionGeometry}.
It contains the exact files used for the 0.8B eight-condition benchmark, the
calm-vs.-pressure reruns, the activation analyses, the steering probes, and
the paper source itself.
